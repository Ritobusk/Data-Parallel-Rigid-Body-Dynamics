...
def autoTreeC (nb : i64) (bf : f64) (skew : f64) (taper : f64) :
    ([nb]i64, [nb]i64, [nb]jointT, [nb]I_Compact, [nb]X_Compact) =
    let (ids, parents', joint_types, Is, Xtree) = autoTree nb bf skew taper
    let Xtree = map to_X_Compact Xtree
    let Is = map to_I_Compact Is
    in (ids, parents', joint_types, Is, Xtree)

def autoVTreeC (nb : i64) (bf : f64) (skew : f64) (taper : f64) :
    ([nb]i64, [nb]i64, [nb]jointT, [nb]I_Compact, [nb]X_Compact, [nb]i64, [nb]i64, []i64, []i64) =
    let (ids, parents', joint_types, Is, Xtree, lp, rp, paths, p_ii1) = autoVTree nb bf skew taper
    let Xtree = map to_X_Compact Xtree
    let Is = map to_I_Compact Is
    in (ids, parents', joint_types, Is, Xtree, lp, rp, paths, p_ii1)
